...
public class Recommend {
     ...
     * @return
     */
    private Map<Double, String> computeNearestNeighbor(String username, Userset.UserSet set) {
        Map<Double, String> distances = new TreeMap<>();

        Userset.UserSet.User u1 = set.getUser(username);
        for (int i = 0; i < set.users.size(); i++) {
            Userset.UserSet.User u2 = set.getUser(i);
     ...
     * @return
     */
    private double pearson_dis(List<Userset.UserSet.Set> rating1, List<Userset.UserSet.Set> rating2) {
        int sum_xy = 0;
        int sum_x = 0;
        int sum_y = 0;
        double sum_x2 = 0;
        ...
        for (int i = 0; i < rating1.size(); i++) {
            Userset.UserSet.Set key1 = rating1.get(i);
            for (int j = 0; j < rating2.size(); j++) {
                Userset.UserSet.Set key2 = rating2.get(j);
    ...
    public List<Userset.UserSet.Set> recommend(String username, Userset.UserSet set) {
        //找到最近邻
        ...
        List<Userset.UserSet.Set> recommendations = new ArrayList<>();

        //找到最近邻看过，但是我们没看过的电影，计算推荐
        Userset.UserSet.User neighborRatings = set.getUser(nearest);
        System.out.println("neighborRatings -> " + neighborRatings.list);

        Userset.UserSet.User userRatings = set.getUser(username);
        System.out.println("userRatings -> " + userRatings.list);

        for (Userset.UserSet.Set artist : neighborRatings.list) {
